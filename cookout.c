...
typedef struct LinkedListObject{
    char *item;
...
/************ prototypes **************/
Object *newObject(char *text);
void insertObject(Object **head, Object *newPointer);
void deleteObject(Object **head, char *text);
void printList(Object **head);

/************ functions ***************/
int main(int argc, char **argv){
    insertObject(&start, newObject("chips"));
    insertObject(&start, newObject("wine"));
    insertObject(&start, newObject("burgers"));
    insertObject(&start, newObject("beer"));
    insertObject(&start, newObject("pizza"));
    insertObject(&start, newObject("zucchini"));
    insertObject(&start, newObject("burgers"));
    insertObject(&start, newObject("slaw"));
    printf("\nINITIAL LIST\n");
    printList(&start);
    deleteObject(&start, "pizza");
    deleteObject(&start, "zucchini");
    deleteObject(&start, "burgers");
    printf("\nALTERED LIST\n");
    printList(&start);
...
Object *newObject(char *text){
    Object *newPointer;
    newPointer = (Object *)malloc(sizeof(Object));
    newPointer->item = (char *)malloc(strlen(text) + 1);
    strcpy(newPointer->item, text);
...
void insertObject(Object **head, Object *newPointer){
    ...
    while((*tracer) && strcmp((*tracer)->item, newPointer->item) < 1){
        tracer = &(*tracer)->next;
    }

    newPointer->next = *tracer;//burgers next = chips
    *tracer = newPointer;// replaces chips with burgers so that beer points at burgers now
}
